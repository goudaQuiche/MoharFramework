...
namespace MoharHediffs
{
    public static class Tools
    {
        ...
        public static BodyPartRecord GetBPRecordWithoutHediff(this Pawn pawn, string bodyPartLabel, HediffDef hd, bool AllowMissing = false, bool myDebug = false)
        {
            ...
            if (bodyPart == null)
            {
                Warn(pawn.Label + "GetBPRecord - did not find " + bodyPartLabel+ " without " + hd.defName, myDebug);
            }
            else
                Warn(pawn.Label + "GetBPRecord - did find " + bodyPartLabel, myDebug);

            return bodyPart;
        }


            public static BodyPartRecord GetBPRecordWithoutHediff(this Pawn pawn, BodyPartDef bodyPartDef, HediffDef hd, bool AllowMissing=false, bool myDebug = false)
        {
            IEnumerable<BodyPartDef> BPDefIE = DefDatabase<BodyPartDef>.AllDefs.Where((BodyPartDef b) => b == bodyPartDef);
            ...
            BodyPartRecord bodyPart = null;
            IEnumerable<BodyPartRecord> BPRList = AllowMissing ? pawn.RaceProps.body.GetPartsWithDef(BPDef) : pawn.health.hediffSet.GetNotMissingParts().Where(bpr => bpr.def == BPDef);
            if (bprToExclude.NullOrEmpty())
            {
                BPRList.TryRandomElement(out bodyPart);
            }
